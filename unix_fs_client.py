...
################## for now RAID5, but GetRecover, Put changes for -cblk 0 #########################
################## this edition print < CORRUPTED_BLOCK virtual_blk > #############################
...
SOCKET_TIMEOUT = 5
RETRY_INTERVAL = 10
# hw5 total  number of servers
TOTAL_SERVER = 1
# hw5 disconnect server number
DISCONNECT_SERVER = -1
# hw5 disconnect server number
CORRUPT_SERVER = -1
# hw5 failure for one block error
ERROR_CORRUPT = -1
# hw5 failure for server disconnect
ERROR_DISCONNECT = -2
...
class DiskBlocks():
    def __init__(self, args):
        ...
        # hw5 set multi server with start point
        if args.startport:
            PORT = args.startport
        else:
            print('Must specify start port number')
            quit()
        # hw 5 define number of servers
        if args.ns:
            global TOTAL_SERVER
            TOTAL_SERVER = args.ns
        else:
            print('Must specify number of server')
            quit()
        for i in range(0, TOTAL_SERVER):
            server_url = 'http://' + SERVER_ADDRESS + ':' + str(PORT)
            # self.block_server[i] = xmlrpc.client.ServerProxy(server_url, use_builtin_types=True)
            self.block_server.insert(i, xmlrpc.client.ServerProxy(server_url, use_builtin_types=True))
            PORT += 1
            logging.debug("INIT: number of server: " + str(TOTAL_SERVER) + ", addr:" + server_url)
    ...
    def SinglePut(self, server_number, block_number, block_data):

        logging.debug('SinglePut: block number ' + str(block_number) + ' len '
                      + str(len(block_data)) + '\n' + str(block_data.hex()))
        if len(block_data) > BLOCK_SIZE:
            logging.error('SinglePut: Block larger than BLOCK_SIZE: ' + str(len(block_data)))
            quit()

        self.server_load[server_number] += 1
        ...
        if block_number in range(0, TOTAL_NUM_BLOCKS):
            ...
            # hw 5 try one time if server disconnect, return to Put() layer
            try:
                # hw 5 call Put() method on the server; code currently quits on any server failure
            ...
            logging.error('SinglePut: Block out of range: ' + str(block_number))
            quit()
    ...
    def SingleGet(self, server_number, block_number):

        logging.debug('SingleGet: ' + str(block_number))

        self.server_load[server_number] += 1

        if block_number in range(0, TOTAL_NUM_BLOCKS):
            # hw 5 try one time if server disconnect, return to Get() layer
            try:
                # call Get() method on the server
                data, tag = self.block_server[server_number].Get(block_number)
                # return as bytearray
                return data, tag
            except ConnectionRefusedError:
                return 0, ERROR_DISCONNECT

        logging.error('SingleGet: Block number larger than TOTAL_NUM_BLOCKS: ' + str(block_number))
        quit()

    ## RSM: read and set memory equivalent

    def SingleRSM(self, server_number, block_number):

        logging.debug('SingleRSM: ' + str(block_number))
        if block_number in range(0, TOTAL_NUM_BLOCKS):
            data = self.block_server[server_number].RSM(block_number)
    ...
    ################# hw 5 build new virtual layer put get rsm for multi server #########################

    # hw5 define a BlockMap method, map virtual block and (server, physical block) for disk read and write
    def VirtualBlockMap(self, virtual_block_number):
        phy_server = virtual_block_number % (TOTAL_SERVER - 1)
        phy_block = virtual_block_number // (TOTAL_SERVER - 1)
        parity_server = (TOTAL_SERVER - 1) - (phy_block % TOTAL_SERVER)
        if phy_server >= parity_server:
            phy_server += 1
        return phy_server, phy_block, parity_server

    # GetParity to get a data of parity block
    def GetParity(self, parity_server, parity_block, virtual_block):

        logging.debug("GetParity:  parity server, block: " + str(parity_server) + ", " + str(parity_block))
        # call Get() method on the server
        data, tag = self.SingleGet(parity_server, parity_block)
        # hw5 resolve block corrupted error -1
        if tag == ERROR_CORRUPT:
            print("CORRUPTED_BLOCK " + str(virtual_block))

            # hw5 use other server block data to get corrupted_parity data
            recover_corrupt = self.GetRecover(parity_server, parity_block)
            return recover_corrupt
        # hw5 resolve disconnect server error -2
        if tag == ERROR_DISCONNECT:
            print("SERVER_DISCONNECTED GET " + str(parity_server))

            # hw5 use other server block data to get disconnected parity server
            recover_disconnect = self.GetRecover(parity_server, parity_block)
            return recover_disconnect

        # if no error, return as bytearray
        return bytearray(data)

    # GetRecover when get has block corrupt or server disconnect error,
    # this will use parity and other server data to recreate one block data
    def GetRecover(self, phy_server, phy_block):
        # phy_server represent server of problem_block or disconnected server, phy_block represents fail_get block
        # use other servers' same physical block data to recreate data
        # if server block is row 0, then we don't use rsm block value do xor
        if phy_block == 0:
            recover_data = bytearray(BLOCK_SIZE)
            for server_number in range(1, TOTAL_SERVER):
                if server_number != phy_server:
                    # we assume only one error happens in the same block_number among server,
                    # if there has another error, this system will corrupt
                    block_data, tag = self.SingleGet(server_number, phy_block)
                    server_data = bytearray(block_data)
                    for i in range(BLOCK_SIZE):
                        recover_data[i] = recover_data[i] ^ server_data[i]
            logging.debug(
                "GetRecover: no use RSM block, recover server block_data: " + str(phy_server) + ", " + str(phy_block))
        else:
            recover_data = bytearray(BLOCK_SIZE)
            for server_number in range(TOTAL_SERVER):
                if server_number != phy_server:
                    # we assume only one error happens in the same block_number among server,
                    # so that this get should be correct without error, if there has another error
                    # if there has another error, this system will corrupt
                    block_data, tag = self.SingleGet(server_number, phy_block)
                    server_data = bytearray(block_data)
                    for i in range(BLOCK_SIZE):
                        recover_data[i] = recover_data[i] ^ server_data[i]
            logging.debug("GetRecover: recover server block_data: " + str(phy_server) + ", " + str(phy_block))
        return recover_data

    def RepairServer(self, server_number):

        total_blk = TOTAL_NUM_BLOCKS // (TOTAL_SERVER - 1)
        for blk_number in range(total_blk):
            blk_data = self.GetRecover(server_number, blk_number)
            ret = self.SinglePut(server_number, blk_number, blk_data)
            if ret == 0:
                logging.debug("Repair: has finished repair server: " + str(server_number))
        return 0
    ...
    def Put(self, block_number, block_data):
        ...
        if block_number in range(0, TOTAL_NUM_BLOCKS):
            ...
            # now implement RAID 5, distribute data and parity block
            phy_server, phy_block, parity_server = self.VirtualBlockMap(block_number)
            logging.debug("Put: vir block: " + str(block_number) + ", physical server, block: "
                          + str(phy_server) + ", " + str(phy_block) + "parity_ser: " + str(parity_server))

            # for RSM_BLOCK 0, (server 0 block 0) if Release()-Put(0, 000000)
            # then if other server set -cblk block is 0, there will have problem, because, when recovery,
            # server0_blk0 is "1", but when release, it set relative parity to 0,
            # then next time before other operation, RSM will set blk to 1, but parity didn't update
            # so for now, when Put is virtual block 0 (RSM BLOCK), we didn't update parity. we omit virtual_blk 0
            if block_number != 0:
                # hw5 implement RAID 5 calculate parity when update block data and store new parity to server

                parity_block = phy_block
                parity_data = self.GetParity(parity_server, parity_block, block_number)
                old_data = self.Get(block_number)
                new_data = bytearray(block_data.ljust(BLOCK_SIZE, b'\x00'))
                xor_data = bytearray(BLOCK_SIZE)

                for i in range(BLOCK_SIZE):
                    xor_data[i] = old_data[i] ^ new_data[i]
                    parity_data[i] = xor_data[i] ^ parity_data[i]
                # logging.info("Parity data: " + str(parity_data))

                ret_parity = self.SinglePut(parity_server, parity_block, parity_data)
                if ret_parity == -1:
                    logging.error('Put: Server returns error')
                    quit()
                if ret_parity == ERROR_DISCONNECT:
                    print("SERVER_DISCONNECTED PUT " + str(parity_server))
                    DISCONNECT_SERVER = parity_server

            ret_data = self.SinglePut(phy_server, phy_block, block_data)
            if ret_data == -1:
                logging.error('Put: Server returns error')
                quit()
                # if singlePut return -2 means that server disconnect
            if ret_data == ERROR_DISCONNECT:
                print("SERVER_DISCONNECTED PUT " + str(phy_server))
                DISCONNECT_SERVER = phy_server
    ...
    def Get(self, block_number):
        global DISCONNECT_SERVER
        global CORRUPT_SERVER
        logging.debug('Get: ' + str(block_number))
        if block_number in range(0, TOTAL_NUM_BLOCKS):

            # now RAID 0, get is from any random server (if all server has same value)
            # server_number = random.randint(0, TOTAL_SERVER - 1)
            # now implement RAID 5, get from mapped server
            phy_server, phy_block, parity_server = self.VirtualBlockMap(block_number)
            logging.debug("Get: vir block: " + str(block_number) + ", physical server, block: "
                          + str(phy_server) + ", " + str(phy_block))
            # call Get() method on the server
            data, tag = self.SingleGet(phy_server, phy_block)
            # hw5 resolve block corrupted error -1
            if tag == ERROR_CORRUPT:
                print("CORRUPTED_BLOCK " + str(block_number))

                # hw5 use parity and other server block data to get corrupted_block data
                recover_corrupt = self.GetRecover(phy_server, phy_block)
                return recover_corrupt

            # hw5 resolve disconnect server error -2
            if tag == ERROR_DISCONNECT:
                print("SERVER_DISCONNECTED GET " + str(phy_server))

                # hw5 use parity and other server block data to get corrupted_block data
                recover_disconnect = self.GetRecover(phy_server, phy_block)
                return recover_disconnect
    ...
    def RSM(self, block_number):
        ...
        if block_number in range(0, TOTAL_NUM_BLOCKS):
            # RSM just put to server 0
            data = self.SingleRSM(0, block_number)
    ...
    def Acquire(self):
        ...
        while lockvalue[0] == 1:  # test just first byte of block to check if RSM_LOCKED
            ...
            lockvalue = self.RSM(RSM_BLOCK);
        # in order to resolve some server -cblk is 0, RSM set block 0 from 0 to 1 to 0 has problem, NOT WORK
        # self.Put(RSM_BLOCK, bytearray(RSM_LOCKED.ljust(BLOCK_SIZE, b'\x01')))
        return 0
            ...
            while current_position < end_position:
                ...
                if entryname_padded == padded_filename:
                    ...
                    if (last_block_index == block_index) and (last_block_end_position == current_position):
                        ...
                        block[current_position:current_position + FILE_NAME_DIRENTRY_SIZE] = bytearray(
                        ...
                        # copy from last
                        block[current_position:current_position + FILE_NAME_DIRENTRY_SIZE] = last_block[
                                                                                             last_block_end_position:last_block_end_position + FILE_NAME_DIRENTRY_SIZE]
                        last_block[
                        last_block_end_position:last_block_end_position + FILE_NAME_DIRENTRY_SIZE] = bytearray(
                            FILE_NAME_DIRENTRY_SIZE)
