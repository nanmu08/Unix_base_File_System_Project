...
#    logging.info('Block (strings in block) [' + str(n) + '] : \n' + str((wholeblock[start:end+1].decode(encoding='UTF-8',errors='ignore'))))
  ...
  # hw5 implement repair
  def repair(self, server):
    try:
      server_id = int(server)
    except ValueError:
      print('Error: ' + server + ' not a valid Integer')
      return -1

    if server_id >= TOTAL_SERVER:
      print("Error: server number should less than :" + str(TOTAL_SERVER - 1))
      return -1
    i = self.FileObject.RawBlocks.RepairServer(server_id)
    return 0

  # print server load distribution
  def serverld(self):
    i = self.FileObject.RawBlocks.server_load
    print("server load is: ", i)
    return 0
